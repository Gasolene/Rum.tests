<?php
	...
	class ImageTestCase extends \System\Testcase\UnitTestCaseBase {
		...
		function test() {
			$img = new \System\Utils\GDImage();
			$img2 = new \System\Utils\GDImage();
			...
			$img2 = $img;
			$img3 = new \System\Utils\GDImage( $img2->getStream() );
			$img3->output( __TMP_PATH__ . '/img.jpg', 'image/jpeg' );
			$img4 = new \System\Utils\GDImage();
            $img4->loadFile(__TMP_PATH__ . '/img.jpg' );
            $img5 = new \System\Utils\GDImage( $img4->getStream() );
            ...
            $img->resize( 8, 30, \System\Utils\GDImageResizeMode::scaleToFit() );
			$this->assertTrue( $img->getWidth() === 8 );
			$this->assertTrue( $img->getHeight() === 6 );

            // __RESIZE__
			$img2->resize( 800, 30, \System\Utils\GDImageResizeMode::resize() );
			$this->assertTrue( $img2->getWidth() === 800 );
			$this->assertTrue( $img2->getHeight() === 30 );

            // __SCALE_TO_WIDTH__
			$img3->resize( 800, 30, \System\Utils\GDImageResizeMode::scaleToWidth() );
			$this->assertTrue( $img3->getWidth() === 800 );
			$this->assertTrue( $img3->getHeight() === 600 );

            // __SCALE_TO_HEIGHT__
			$img4->resize( 800, 30, \System\Utils\GDImageResizeMode::scaleToHeight() );
			$this->assertTrue( $img4->getWidth() === 40 );
			$this->assertTrue( $img4->getHeight() === 30 );

            // __SCALE_TO_CROP__
			$img5->resize( 200, 30, \System\Utils\GDImageResizeMode::scaleToCrop() );
			$this->assertTrue( $img5->getWidth() === 200 );
			$this->assertTrue( $img5->getHeight() === 150 );

			$img5->resize( 10, 15, \System\Utils\GDImageResizeMode::scaleToCrop() );
